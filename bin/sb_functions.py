...
def compute_cohort_centralities(objects, markers, phenotyping_column='cellType', radius=35):
    ...
    for i, imagename in enumerate(imagenames):
        ...
        if len(image_cTypes) > 0: # only proceed if there are cells of the specified phenotype level in the image
            ...
            adata = AnnData(features, obsm={"spatial": coords}, obs={f'{phenotyping_column}':cellTypes})
            print(adata)
...
def compute_spatial_graph(objects, imagename, markers, phenotyping_column='cellType', radius=35, rand_features=True):
...
def compute_nn_graph(objects, imagename, markers, phenotyping_column='cellType', n_neighs=6, rand_features=True):
...
def min_path_to_epithelial(shortest_paths, phenotyping_column='cellType'):
    min_path = shortest_paths[shortest_paths[phenotyping_column] == 'Epithelial cells']['distance'].min()
    return int(min_path)

def min_path_to_cellType(shortest_paths, phenotyping_column, cType):
    min_path = shortest_paths[shortest_paths[phenotyping_column] == cType]['distance'].min()
    return int(min_path)


def get_predecessor(df, minpath_to_target, phenotyping_column, target_cellType):
    print(df)
    predecessor = df[(df['distance'] == minpath_to_target) & (df[phenotyping_column] == target_cellType)]['predecessor'].values.tolist()
...
def followchain(shortest_paths, minpath_to_target, source_cell, source_vertex, target_cell = 'Epithelial cells', phenotyping_column='cellType'):
    '''
    ...
    sorted_shortest_paths = sorted_shortest_paths[(sorted_shortest_paths['distance'] == minpath_to_target) & (sorted_shortest_paths[phenotyping_column] == target_cell)]
    ...
    if minpath_to_target == 1:
        ...
        if sorted_shortest_paths['predecessor'].max() >= 0: # possible to have a df with target cells that are disconnected
            ...
            predecessors = get_predecessor(sorted_shortest_paths, minpath_to_target, phenotyping_column, target_cellType = target_cell) # get list of initial predecessor cells            
            p = predecessors[0]
            ...
            # take first predecessor if more than one:
                ...
                if p != -1: ## proceed only if the predecessor is not the source cell
                    predecessor_nodeType = get_predecessor_nodeType(shortest_paths, p, phenotyping_column=phenotyping_column)
...
def degenerate_path_content(closest_epi, shortest_paths, minpath_to_epi, barrier_cells=['Myofibroblasts'], phenotyping_column='cellType'):
    ...
    if minpath_to_epi > 1: #
        ...
        for i in range(len(closest_epi)):
            ...
            if set(barrier_cells).intersection(set(degenerate_path_content[phenotyping_column])) != set():
                myofibroblast_fraction_degen = degenerate_path_content[phenotyping_column].value_counts(normalize=True)[barrier_cells].sum()
            else:
...
def degenerate_adjacent_barrier(closest_epi, shortest_paths, minpath_to_epi, barrier_cells=['Myofibroblasts'], phenotyping_column='cellType'):
    ...
    if minpath_to_epi > 1: #
        ...
        for i in range(len(closest_epi)):
            ...
            if set(barrier_cells).intersection(set(adjacent_cells_df[phenotyping_column])) != set():                
                mean_adjacent_barrier = adjacent_cells_df[phenotyping_column].value_counts(normalize=True)[barrier_cells].sum() #/ len(adjacent_cells_df)
            else:
...
def permute_phenotypes(objects, in_place=True, region = 'all', phenotyping_column = 'cellType'):
    '''
        ...
        for imagename in objects['imagename'].unique():
            ...
            if in_place:
                objects.loc[objects['imagename'] == imagename, 
                            (phenotyping_column, 'majorType', 'positive')] = rng.permutation(objects.loc[objects['imagename'] == imagename,
                                                                                                 (phenotyping_column, 'majorType', 'positive')].values)
            else:
                objects.loc[objects['imagename'] == imagename, 
        ...
        for imagename in objects['imagename'].unique():
            ...
            if in_place:
                objects.loc[(objects['imagename'] == imagename) & (objects['region'] == region), 
                            (phenotyping_column, 'majorType', 'positive')] = rng.permutation(objects.loc[(objects['imagename'] == imagename) & (objects['region'] == region),
                                                                                                 (phenotyping_column, 'majorType', 'positive')].values)
            else:
                objects.loc[(objects['imagename'] == imagename) & (objects['region'] == region), 
