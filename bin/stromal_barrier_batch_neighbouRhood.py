...
def main(args):
    ...
    if imagename in objects['imagename'].unique():
                    ...
                    if v >= 0: # Starting vertex should be between 0 to number of vertices
                        ...
                        if minpath_to_epi < 1000:
                            ...
                            if CALC_CHAIN == True:
                                ...
                                cellchain, vertexes = followchain(shortest_paths, minpath_to_epi, source_cell=cellType, source_vertex=v, phenotype_level=phenotyping_level)
        ...
        image_barrier_df['object_chain'] = image_barrier_df['vertex_chain'].apply(lambda x: relabel_vertex_chain(x, node_label_dict))

        ## add marker positivity information:
        positive_dict = dict(zip(node_ids['vertex'], node_ids['positive']))
        image_barrier_df['positivity_chain'] = image_barrier_df['vertex_chain'].apply(lambda x: relabel_vertex_chain(x, positive_dict))
